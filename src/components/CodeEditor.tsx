...
`,
  ...
  {
    id: 'variables-ds',
    name: '2. 変数を使う (DS擬似命令による記憶領域の確保)',
    code: `; === DS擬似命令で「変数」を定義して使うデモ ===
; HLASMのDS命令に倣い、メモリ上に名前付きの記憶領域を明示的に確保します。
; 確保した変数は名前で [変数名] のように読み書きできます。
; ※ DSはコード末尾(HALTの後)にまとめて配置するのが定石です。

; --- カウンタ変数を10からデクリメントしながら合計を求める ---
LOAD R0, 10
STORE [counter], R0   ; counter = 10
LOAD R1, 0
STORE [sum], R1       ; sum = 0

loop:
LOAD R1, [sum]        ; 変数 sum をロード
LOAD R0, [counter]    ; 変数 counter をロード
ADD R1, R0            ; sum += counter
STORE [sum], R1       ; 結果を変数 sum に書き戻す

SUB R0, 1             ; counter -= 1
STORE [counter], R0
CMP R0, 0
BNE loop              ; counter が 0 でなければ繰り返し

; ここで sum には 10+9+...+1 = 55 が格納されている
LOAD R2, [sum]        ; 確認用に R2 へ読み出し
HALT

; === データ領域 (変数定義) ===
counter: DS X         ; 1バイトのカウンタ変数
sum:     DS X         ; 1バイトの合計値変数
`,
  },
  ...
  {
    id: 'dat-basic',
    name: '3. DATによる仮想メモリの魔術 (メモリ切り替え & 全アクセス検証)',
...
; 2. DATの設定をして有効化
; ★まずページテーブルの置き場所を決めます。CR1の初期値0のままだと
;   テーブルが物理0x000~0x003＝このプログラムの先頭4バイトの上に書かれてしまうため、
;   空いている物理フレーム6(0x180~)へ逃がします。
LOAD R7, 6
LCTL CR1, R7    ; ページテーブルを物理フレーム6に置く
DATSET 0, 0     ; ★ページ0(0x00~0x3F) -> 物理フレーム0（実行中のコード自身）
                ;   命令フェッチもDATを通るので、自分のコードのページも必ずマップする！
...
; 7. DATを無効化して直接アクセス
DATDIS          ; DATを無効化！
LOAD R3, [0x40] ; R3 = 物理RAMの0x40から直接ロード
                ; ★同じ [0x40] でも、DAT無効なら物理0x40そのもの。
                ;   このプログラムは0x40より手前で終わっているので、読まれるのは 0 (空き領域)。
...
`,
  ...
  {
    id: 'context-switch',
    name: '4. コンテキストスイッチ (2プロセスの共存)',
    code: `; === コンテキストスイッチ：2つのプロセスが同じ仮想アドレスを使う ===
; プロセスAとプロセスBに専用のページテーブルを用意し、CR1を切り替えるだけで
; 「見えるメモリ」を丸ごと入れ替えます。
; 両者とも同じ仮想アドレス 0x40 に書き込むのに、実体は別々の物理フレームに
; 保存されるので、互いのデータを壊しません。

; --- 1. プロセスAのページテーブルを構築 (物理フレーム1に置く) ---
LOAD R0, 1
LCTL CR1, R0    ; これから作る表の置き場所 = フレーム1
DATSET 0, 0     ; ページ0 -> フレーム0 (実行中のコード自身。これを忘れるとFAULT!)
DATSET 1, 3     ; ページ1 (仮想0x40~) -> フレーム3 (物理0x0C0~) = Aのデータ領域

; --- 2. プロセスBのページテーブルを構築 (物理フレーム2に置く) ---
LOAD R0, 2
LCTL CR1, R0    ; これから作る表の置き場所 = フレーム2
DATSET 0, 0     ; ページ0 -> フレーム0 (コードは共通)
DATSET 1, 4     ; ページ1 (仮想0x40~) -> フレーム4 (物理0x100~) = Bのデータ領域

; --- 3. プロセスAとして実行 ---
LOAD R0, 1
LCTL CR1, R0    ; ★プロセスAへコンテキストスイッチ (見る表を切り替えるだけ)
DATEN           ; DATを有効化！
LOAD R1, 10
STORE [0x40], R1 ; Aが仮想0x40に 10 を書く -> 実体は物理0x0C0

; --- 4. プロセスBへ切り替えて実行 ---
LOAD R0, 2
LCTL CR1, R0    ; ★プロセスBへコンテキストスイッチ
LOAD R1, [0x40] ; Bから見た仮想0x40 = 物理0x100 なので、まだ 0 (Aの10は見えない)
ADD  R1, 99
STORE [0x40], R1 ; Bが仮想0x40に 99 を書く -> 実体は物理0x100

; --- 5. プロセスAへ復帰して確認 ---
LOAD R0, 1
LCTL CR1, R0    ; ★プロセスAへ戻る
LOAD R2, [0x40] ; R2 = 10 … Bに上書きされず、Aのデータは無事！
...
`,
  },
  {
    ...
    name: '5. 総合検証パレード (全命令＆データパス描画テスト)',
...
LOAD R7, 6       ; ページテーブルの置き場所 = 物理フレーム6 (0x180~)
LCTL CR1, R7     ; ★CR1が0のままだと、表がプログラム先頭4バイトの上に書かれてしまう
DATSET 0, 0      ; ★ページ0 -> フレーム0 (実行中のコード自身を素通しでマップ)
                 ;   命令フェッチもDATを通るので、これが無いとDATEN直後にFAULTする
