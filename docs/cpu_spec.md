...
### 4.1 FETCH (フェッチ)
1. 現在の `PC`（仮想アドレス）を**DATで変換した物理RAMの番地**からオペコード（1バイト目）を読み取ります（§2.4 参照）。
2. その命令に必要なサイズ（1〜3バイト）分を、**1バイトずつDAT変換しながら**物理RAMからフェッチバッファ（`fetchBuffer`）に読み込みます。
   * したがって **命令フェッチもページフォルトの対象**です。DAT有効化後は、実行中のコードが乗っているページのマッピングも必ず用意しておく必要があります。
...
以下は、DATを設定し、物理フレーム7（RAMのアドレス `0x1C0`〜）に対してデータを読み書きするサンプルプログラムです。

**注意**: §2.4 / §4.1 のとおり **命令フェッチもDATを通ります**。`DATEN` の時点で、実行中のコードが乗っているページ（ここではページ0）のマッピングが無いと、直後の命令フェッチでページフォルトして `FAULT` します。データ用のページだけをマップして `DATEN` してはいけません。

```assembly
; === DATの初期設定サンプル ===
LOAD R7, 6        ; ページテーブルの置き場所 = 物理フレーム6 (0x180~)
LCTL CR1, R7      ; CR1 が 0 のままだと、表がプログラム先頭4バイトの上に書かれてしまう

DATSET 0, 0       ; 仮想ページ0 -> 物理フレーム0 (実行中のコード自身をそのまま見せる)
DATSET 1, 7       ; 仮想ページ1 (0x40~0x7F) -> 物理フレーム7 (RAM 0x1C0~0x1FF) = データ用
DATEN             ; DATを有効化

LOAD R0, 42       ; 書き込むデータ (42) をR0にロード
STORE [0x45], R0  ; 仮想アドレス 0x45 (= ページ1のオフセット5 = 物理 0x1C5) に書き込む

LOAD R1, [0x45]   ; 仮想アドレス 0x45 から R1 にロード (R1は 42 になる)

HALT              ; 実行を停止
```

実行結果: `HALTED` / `R0 = R1 = 42` / 物理RAM `0x1C5 = 42 (0x2A)` / ページテーブル実体は `0x180 = 0x80`, `0x181 = 0x87`。
...
DATSET 0, 0       ; ページ0 -> 物理フレーム0 (実行中のコード自身。これが無いとFAULT)
DATSET 1, 3       ; ページ1 -> 物理フレーム3 にマップ

; 2. プロセスBのテーブル構築 (RAMフレーム2)
LOAD R0, 2        
LCTL CR1, R0      ; CR1にフレーム2をセット
DATSET 0, 0       ; ページ0 -> 物理フレーム0 (コードは両プロセス共通)
DATSET 1, 4       ; ページ1 -> 物理フレーム4 にマップ

; 3. プロセスAを実行
LOAD R0, 1        
LCTL CR1, R0      ; プロセスAへコンテキストスイッチ！
DATEN             ; DAT有効化 (Aの表が有効になってから初めて有効化する)
LOAD R1, 10       
...
実行結果: `HALTED` / `R2 = 10` / 物理RAM `0x0C0 = 10`（Aのデータ）, `0x100 = 99`（Bのデータ）。
同じ仮想アドレス `0x40` への書き込みが、表の切り替えだけで別々の物理フレームに振り分けられていることが確認できます。
